...
Modified for python 3 + Some added functions
"""

import collections, re, glob, os
import copy
...
class Tree(dict):
	...
	# def addspans(self, exclude=[]):
	# 	"""
	# 	adds dictionary to each node: 'kids': {1: 'dobj', 2: 'aux', 3: 'nsubj', 5: 'prep', 9: 'punct'}
	# 	"""
	# 	self.addkids()
	# 	for i in self:
	# 		for k in self[i]["kids"]:

	
	def get_kids(self, i):
		return list(self[i]["kids"].keys())
...
def conllFolder2trees_unpuncted(folder):
	"""
	path to folder containing files in .conllu -> list of trees
	...
	trees = list()
	for fichier in fichiers:
		subtrees = conllFile2trees(fichier)
		for t in subtrees:
			tree = unpunctATree_2(t)
			trees.append(tree)
	return trees

def UD_unpunct(folder, outfolder):
	treebanks = glob.glob(folder)
	for num, t in enumerate(treebanks):
		print(num)
		newdir = os.path.join(outfolder, t.split("/")[-1])
		print(newdir)
		fichiers = glob.glob(t+"/*.conllu")
		# print(fichiers)
		...
		for f in fichiers:
			newtrees = list()
			oldtrees = conllFile2trees(f)
			base = "/".join(f.split("/")[-2:])
			newpath = os.path.join(outfolder, base)
			# print(newpath)
			for old in oldtrees:
				new = unpunctATree_2(old)
				newtrees.append(new)
			trees2conllFile(newtrees, newpath)
		

if __name__ == "__main__":
	conll = """1-2	彭拿路	彭拿路	PROPN	NNP	_	2	nsubj	_	SpaceAfter=No
1	彭拿路	彭拿路	PROPN	NNP	_	2	nsubj	_	SpaceAfter=No
